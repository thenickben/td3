...
class TD3(object):
	...
	def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2, beta = 0.):
		...
		for it in range(iterations):
			...
			if self.PER:
				...
				critic_loss = torch.mean(weights * (target_Q - current_Q1) ** 2) + torch.mean(weights * (target_Q - current_Q2) ** 2)
			else:
